...
const Enterprise = () => {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-pink-50 relative overflow-hidden transition-opacity duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <nav className={`fixed top-0 left-0 right-0 bg-white border-b border-purple-200/50 z-50 shadow-sm transform transition-all duration-700 ${isLoaded ? 'translate-y-0' : '-translate-y-full'}`}>
      ...
      <div className="pt-24 pb-16 px-12 md:px-16 lg:px-20 relative z-10">
        <div className="max-w-full mx-auto scale-[0.8] origin-top">
          {/* Back Button */}
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-20">
            {features.map((feature, index) => (
              <div 
                key={index} 
                className={`group bg-white p-8 rounded-2xl border-2 border-purple-200/50 hover:border-purple-400 hover:shadow-2xl hover:shadow-purple-500/20 transition-all duration-500 transform hover:-translate-y-2 animate-fade-in-up ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}
          ...
          {/* Contact Form */}
          <div className={`bg-white rounded-2xl border-2 border-purple-200/50 p-10 shadow-2xl animate-fade-in-up transform transition-all duration-700 delay-600 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
            <div className="text-center mb-8">
